...
def scatter_criteria(clustered_dataset, attributes, numerics, nominals):
    ...
    #print(np.linalg.inv(within_scatter), '\n')
...
#    print('evaluation metric:', evaluation_metric)
    ...
    return 1, singular_counter #evaluation_metric,


## remove the ID column if ID is included
if ID_attribute in dataset_array[0]:
    id_index = list(dataset_array[0]).index('id')
    dataset_array = np.delete(dataset_array, id_index, 1)
elif ID_attribute not in dataset_array[0]:
    dataset_array = dataset_array


attributes = numerical_attribute_set + categorical_attribute_set
numerics = numerical_attribute_set
nominals = categorical_attribute_set

nominal_indices = []
for attr in attributes:
    if attr in nominals:
        nominal_index = list(dataset_array[0]).index(attr)
        nominal_indices.append(nominal_index)

#normalized_data = normalizing_data(grouped_data, categorical_attribute_set)
### imputing missing data in the data set
#completed_data = completing_by_KNN(dataset_array, dataset_array, categorical_attribute_set)

data_set = cp.deepcopy(dataset_array[1:])
#
#ranked_attributes = rank_features(data_set, attributes, numerics, nominals)
#for item in ranked_attributes: print(item)

#ranked_attributes = ['arbeidsevne_1', 'tampa_1item', 'keelestartback_risk_1', 'sovn_1', 'fdselsr']
ranked_attributes = ['arbeidsevne_1', 'keelestartback_risk_1',  'smertevarighet', 'orebro_total_1', 'aktivitetsnivaa_1']
do_clustering(data_set, attributes, ranked_attributes, numerics, nominals, n_base_attr)    

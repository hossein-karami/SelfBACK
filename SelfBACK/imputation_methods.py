...
# This function delete cases with incomplete data
...
def imputation_error_checking(normalized_data, clustering_attribute_set, nominal_attributes):
#    for column in range(1,len(clustering_attribute_set)+1):
    for column in clustering_attribute_set:
      
        temp_dataset = list(normalized_data[:])
        
        num_of_row = len(temp_dataset)
        ...
        for i in range(1,iteration_num+1):
            rand_row_index = randint(1, num_of_row-1)
            rand_column_index = list(temp_dataset[0]).index(column)
            
            checking_value = temp_dataset[rand_row_index][rand_column_index]
            
            temp_dataset[rand_row_index][rand_column_index] = None
            complete_data = completing_by_KNN_error_checking_version(normalized_data, temp_dataset, nominal_attributes, rand_row_index, rand_column_index)
            
            if complete_data[rand_row_index, rand_column_index] == checking_value and checking_value != None:
                correct_guess_num += 1
                
            elif complete_data[rand_row_index, rand_column_index] != checking_value and checking_value != None:
                wrong_guess_num += 1
                error_value = error_value + abs(complete_data[rand_row_index, rand_column_index] - checking_value)
                #print('picked value:', checking_value, ', estimated value:', complete_data[rand_row_index, rand_column_index])
                error_values_list.append(round(abs(complete_data[rand_row_index, rand_column_index] - checking_value) , 2))
        if wrong_guess_num != 0:
            error_value = error_value/wrong_guess_num
        
        error_values_dic = Counter(error_values_list)
        
        accuracy = float(correct_guess_num / (correct_guess_num + wrong_guess_num))
        print('accuracy of', temp_dataset[0][rand_column_index], 'is about: ', accuracy)
        for key, value in error_values_dic.items():
            print(key, value)
        print('number of estimations:', correct_guess_num + wrong_guess_num)
